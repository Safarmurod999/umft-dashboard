...
const Position = () => {
    ...
    const position = useSelector((state) => state.data.data);
    const isLoading = useSelector((state) => state.data.isLoading);
    const error = useSelector((state) => state.data.error);

    useEffect(() => {
        dispatch(fetchData({ apiEndpoint: "position", accessToken }));
    }, [dispatch, pathname]);

    const deletePosition = (id) => {
        dispatch(deleteData({ apiEndpoint: "position", id, accessToken }));
        toast.error("Mahsulot o'chirildi");
    };
    const updatePosition = (data) => {
        let newData = data;
        dispatch(updateData({ apiEndpoint: "position", newData, accessToken }));
        toast.success("Mahsulot muvaffaqiyatli o'zgartirildi!");
    };
    const addposition = (e) => {
        e.preventDefault()
        let newData = { title: data };
        dispatch(addData({ apiEndpoint: "position", newData, accessToken }));
            ...
            <section className='pt-[104px]'>
                ...
                <div className="relative overflow-x-auto max-w-[95%] mx-auto">
                    <table className="w-full text-sm text-left rtl:text-right text-gray-500 rounded-lg border">
                        ...
                        <tbody>
                            {
                                position.length > 0 ? position.map((el) => (
                                        ...
                                        <td className='space-x-[10px]'>
                                            {
                                                ...
                                                >
                                                    ...
                                                    <button
                                                        ...
                                                        className="focus:outline-none text-white bg-white focus:ring-1 focus:ring-blue-300 font-medium rounded-lg text-sm px-2 py-2 ring-blue-300 ring-1"
                                                        onClick={() => {
                                                            updatePosition({ _id: el._id, title: edit });
                                                            setEditMode(null);
                                                        }}
                                                    >
                                ...
                                )) : <tr><td colSpan="5" className="py-5 text-center text-4xl">No data</td></tr>
                            }
                        </tbody>
                    </table>
                </div>
                <div className="max-w-[95%] mx-auto">
                    <form className='flex space-x-[10px] items-center' onSubmit={addposition}>
