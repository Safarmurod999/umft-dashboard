...
const Spec = () => {
    ...
    const spec = useSelector((state) => state.data.data);
    const isLoading = useSelector((state) => state.data.isLoading);
    const error = useSelector((state) => state.data.error);

    useEffect(() => {
        dispatch(fetchData({ apiEndpoint: "spec", accessToken }));
    }, [dispatch, pathname]);

    const deleteSpec = (id) => {
        dispatch(deleteData({ apiEndpoint: "spec", id, accessToken }));
        toast.error("Mahsulot o'chirildi");
    };
    const updateSpec = (data) => {
        let newData = data;
        dispatch(updateData({ apiEndpoint: "spec", newData, accessToken }));
        toast.success("Mahsulot muvaffaqiyatli o'zgartirildi!");
    };
    const addSpec = (e) => {
        e.preventDefault()
        let newData = { title: data };
        dispatch(addData({ apiEndpoint: "spec", newData, accessToken }));
        ...
        spec && (
            <section className='pt-[104px]'>
                                        ...
                                        <td className='space-x-[10px]'>
                                            {
                                                ...
                                                >
                                                    ...
                                                    <button
                                                        ...
                                                        className="focus:outline-none text-white bg-white focus:ring-1 focus:ring-blue-300 font-medium rounded-lg text-sm px-2 py-2 ring-blue-300 ring-1"
                                                        onClick={() => {
                                                            updateSpec({ _id: el._id, title: edit });
                                                            setEditMode(null);
                                                        }}
                                                    >
                ...
                <div className="max-w-[95%] mx-auto">
                    <form className='flex space-x-[10px] items-center' onSubmit={addSpec}>
